...
const Dashboard = ({ profile, plan, onReset }: { profile: UserProfile, plan: PlanResult, onReset: () => void }) => {
  ...
  const renderAbout = () => (
    <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="bg-white p-8 rounded-3xl border border-[#E5E5E5] shadow-sm text-center">
        <img 
          src="https://raw.githubusercontent.com/olivervivi/alimentacao-e-exercicio/main/public/perfil.jpg" 
      ...
      <div className="bg-[#FAFAFA] p-6 rounded-2xl border border-[#E5E5E5] shadow-inner">
        ...
        {showCopySuccess && (
          <div className="mt-3 text-center animate-in fade-in slide-in-from-top-2 duration-300">
            <span className="inline-flex items-center gap-2 bg-[#E0F2F1] text-[#00695C] px-4 py-2 rounded-lg text-sm font-bold border border-[#B2DFDB]">
              <CheckCircle2 size={16} /> Plano copiado com sucesso!
            </span>
          </div>
        )}
      </div>
      ...
      <div className="bg-[#EDF6F9] p-6 rounded-3xl border border-[#83C5BE]/20 flex flex-col sm:flex-row items-center gap-6 shadow-sm mt-8">
        ...
        <div className="flex-1 text-center sm:text-left">
          ...
          <p className="text-xs text-[#264653]/70 mb-4 leading-relaxed">
            Este aplicativo é gratuito. Se ele te ajudou e você quiser apoiar a continuidade do projeto, pode fazer uma contribuição voluntária.
            <br/><br/>
            Você pode apoiar utilizando o QR Code ou a chave PIX abaixo.
          </p>
          <div className="bg-white border border-[#E29578]/30 rounded-xl p-2 flex items-center justify-between gap-3 hover:border-[#E29578]/50 transition-colors group cursor-pointer" onClick={() => {
            navigator.clipboard.writeText(pixKey);
            setShowPixCopySuccess(true);
            setTimeout(() => setShowPixCopySuccess(false), 3000);
          }}>
            <p className="font-mono text-[#264653] text-[10px] truncate flex-1 px-2"><span className="font-bold text-[#E29578] mr-1">Chave PIX:</span> {pixKey}</p>
  ...
  return (
    <div className="flex flex-col md:grid md:grid-cols-[220px_1fr] lg:grid-cols-[260px_1fr] gap-8 items-start w-full max-w-none relative pb-24 md:pb-0 min-h-screen">
      
      {/* Desktop Sidebar (Hidden on Mobile) */}
      <aside className="hidden md:flex w-full shrink-0 bg-white rounded-3xl border border-[#83C5BE]/30 p-4 shadow-sm sticky top-24 min-h-[80vh] flex-col">
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-[#83C5BE]/30 z-50 px-4 py-2 flex overflow-x-auto no-scrollbar gap-2 items-center shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
        {menuItems.map((item) => (
          <button
            key={item.id}
            disabled={item.disabled}
      ...
      {/* Main Content Area */}
      <main className="w-full min-w-0 overflow-x-auto bg-white rounded-3xl border border-[#83C5BE]/30 p-6 md:p-8 shadow-sm min-h-[500px]">
        ...
        <div className="mt-8 text-center max-w-[600px] mx-auto text-black text-sm">
          <p className="font-medium leading-relaxed">Baseado em evidências científicas e diretrizes de saúde (Atualização 2026).</p>
          <p className="mt-2 leading-relaxed">© 2026 Alimentação e Exercício.</p>
        </div>
      </main>
    </div>
  );
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#EDF6F9] font-sans text-[#264653] pb-10">
      ...
      <main className="max-w-[clamp(1000px,90vw,1400px)] mx-auto px-4 py-6 md:py-8">
